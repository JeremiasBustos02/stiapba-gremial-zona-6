...
export function HistoryPage({ role, onUseAsBase, openDocumentId, onDocumentOpened }: { role: 'ADMIN' | 'DELEGADO'; onUseAsBase?: (detail: DocumentHistoryDetail) => void; openDocumentId?: string; onDocumentOpened?: () => void }) {
  ...
  return <section className="max-w-[82rem]">
     <header className="sm:grid sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start sm:gap-x-8">
       ...
       <div className="mt-5 sm:mt-0 sm:justify-self-end"><Button variant="outline" className="min-h-11 w-fit" onClick={() => void exportHistory()} disabled={!historyQuery.data || historyQuery.data.totalElements === 0 || exporting}><FileSpreadsheet size={17} aria-hidden="true" /> {exporting ? <><LoaderCircle className="animate-spin" size={17} aria-hidden="true" /> Exportando...</> : 'Exportar'}</Button><p aria-live="polite" className="typo-meta mt-2 tabular-nums text-slate-600 sm:text-right">{historyQuery.data ? `${historyQuery.data.totalElements} ${historyQuery.data.totalElements === 1 ? 'documento' : 'documentos'}` : ''}</p><p className="typo-meta mt-1 max-w-56 text-slate-500 sm:text-right">Se exportarán todos los documentos que coinciden con los filtros actuales.</p></div>
     </header>
     <section aria-label="Buscar y filtrar historial" className="mt-7 border-t border-slate-200 pt-5">
       <div className="flex flex-col gap-3 lg:hidden"><div className="min-w-0"><Label htmlFor="history-search">Buscar</Label><div className="relative mt-2"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={18} aria-hidden="true" /><Input id="history-search" value={filters.q} onChange={(event) => updateFilters({ q: event.target.value })} placeholder="Buscar documentos" className="pl-10" /></div></div><Button variant="outline" className="min-h-11 w-fit px-3" aria-expanded={filtersOpen} onClick={() => setFiltersOpen(true)}><SlidersHorizontal size={18} aria-hidden="true" /> Filtros</Button></div>
       <div className="hidden gap-3 lg:grid lg:grid-cols-[minmax(18rem,1.8fr)_repeat(3,minmax(10rem,1fr))] lg:items-end">
        <div className="hidden lg:block"><Label htmlFor="history-search-desktop">Buscar</Label><div className="relative mt-2"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={18} aria-hidden="true" /><Input id="history-search-desktop" value={filters.q} onChange={(event) => updateFilters({ q: event.target.value })} placeholder="Número, empresa o delegado" className="pl-10" /></div></div>
        <div><Label htmlFor="history-from">Fecha desde</Label><Input id="history-from" className="mt-2" type="date" value={filters.issueDateFrom} onChange={(event) => updateFilters({ issueDateFrom: event.target.value })} /></div>
        <div><Label htmlFor="history-to">Fecha hasta</Label><Input id="history-to" className="mt-2" type="date" value={filters.issueDateTo} onChange={(event) => updateFilters({ issueDateTo: event.target.value })} /></div>
        {role === 'ADMIN' && <div><Label htmlFor="history-created-by">Generado por</Label><Input id="history-created-by" className="mt-2" value={filters.createdBy} onChange={(event) => updateFilters({ createdBy: event.target.value })} placeholder="Nombre" /></div>}
        <div><Label htmlFor="history-order">Orden</Label><Select id="history-order" className="mt-2" value={filters.order} onChange={(event) => updateFilters({ order: event.target.value as DocumentHistoryFilters['order'] })}><option value="newest">Más recientes</option><option value="oldest">Más antiguos</option></Select></div>
      </div>
    ...
    {historyQuery.isPending ? <LoadingState /> : historyQuery.isError ? <ErrorState text={errorText} onRetry={() => historyQuery.refetch()} /> : historyQuery.data.content.length === 0 ? <EmptyState role={role} filtered={hasFilters} onClear={clearFilters} /> : <>
       <div className="mt-6 hidden overflow-visible border-y border-slate-200 bg-white md:block"><Table><TableHeader><TableRow><TableHead className="h-10 px-3">Número</TableHead><TableHead className="h-10 px-3">Generación</TableHead><TableHead className="h-10 px-3">Tipo</TableHead><TableHead className="h-10 px-3">Empresa</TableHead><TableHead className="h-10 px-3">Delegado</TableHead><TableHead className="h-10 px-3">Fecha permiso</TableHead><TableHead className="h-10 px-3 text-right">Acciones</TableHead></TableRow></TableHeader><TableBody>{historyQuery.data.content.map((record) => <HistoryRow key={record.id} record={record} downloading={downloadingId === record.id} onDetail={() => setSelected(record)} onView={() => view(record)} onDownload={() => download(record)} />)}</TableBody></Table></div>
        <div className="mt-5 divide-y divide-slate-200 border-y border-slate-200 md:hidden">{historyQuery.data.content.map((record) => <HistoryItem key={record.id} record={record} downloading={downloadingId === record.id} onDetail={() => setSelected(record)} onView={() => view(record)} onDownload={() => download(record)} />)}</div>
...
function HistoryRow({ record, downloading, onDetail, onView, onDownload }: { record: DocumentHistoryRecord; downloading: boolean; onDetail: () => void; onView: () => void; onDownload: () => void }) { return <TableRow><TableCell className="px-3 py-2"><button type="button" className="font-semibold text-blue-800 underline-offset-4 hover:underline" onClick={onDetail}>{record.publicNumber}</button></TableCell><TableCell className="px-3 py-2 text-slate-600">{formatDateTime(record.createdAt)}</TableCell><TableCell className="px-3 py-2"><Badge className="whitespace-nowrap">{documentTypeLabels[record.documentType] ?? record.documentType}</Badge></TableCell><TableCell className="px-3 py-2 font-medium text-slate-800">{record.companyName}</TableCell><TableCell className="px-3 py-2">{record.delegateName}</TableCell><TableCell className="px-3 py-2 text-slate-600">{formatDate(record.issueDate)}</TableCell><TableCell className="px-3 py-2 text-right"><HistoryActions record={record} downloading={downloading} onDetail={onDetail} onView={onView} onDownload={onDownload} /></TableCell></TableRow> }
function HistoryActions({ record, downloading, onDetail, onView, onDownload }: { record: DocumentHistoryRecord; downloading: boolean; onDetail: () => void; onView: () => void; onDownload: () => void }) {
  ...
  return <div ref={root} className="relative inline-flex text-left"><Button ref={trigger} type="button" variant="ghost" aria-label="Acciones" aria-haspopup="menu" aria-expanded={open} aria-controls={menuId} className="min-h-11 gap-1.5 px-3 text-slate-700 hover:bg-slate-100" onClick={() => setOpen((current) => !current)} onKeyDown={openFromKeyboard}><span className="hidden md:inline">Acciones</span><ChevronDown className="hidden md:block" size={16} aria-hidden="true" /><MoreHorizontal className="md:hidden" size={19} aria-hidden="true" /></Button>{open && <div id={menuId} role="menu" aria-label={`Acciones de ${record.publicNumber}`} className="absolute right-0 top-[calc(100%+0.25rem)] z-20 min-w-44 border border-slate-200 bg-white p-1 shadow-sm"><button type="button" role="menuitem" className="flex min-h-10 w-full items-center px-3 text-left text-sm font-medium text-slate-800 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-600" onClick={() => run(onDetail)}>Ver detalle</button><button type="button" role="menuitem" className="flex min-h-10 w-full items-center px-3 text-left text-sm font-medium text-slate-800 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-600" disabled={downloading} onClick={() => run(onView)}>Ver PDF</button><button type="button" role="menuitem" className="flex min-h-10 w-full items-center px-3 text-left text-sm font-medium text-slate-800 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-600 disabled:cursor-not-allowed disabled:opacity-60" disabled={downloading} onClick={() => run(onDownload)}>Descargar</button><DisabledEmailAction label="Enviar" className="block [&_button]:min-h-10 [&_button]:border-0 [&_button]:px-3 [&_button]:text-left [&_button]:shadow-none" /></div>}</div>
}
function HistoryItem({ record, downloading, onDetail, onView, onDownload }: { record: DocumentHistoryRecord; downloading: boolean; onDetail: () => void; onView: () => void; onDownload: () => void }) { return <article className="bg-[#fffdf8] py-3 first:pt-0 last:pb-0"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><button type="button" className="typo-control min-h-11 text-blue-800 underline-offset-4 hover:underline" onClick={onDetail}>{record.publicNumber}</button><Badge className="ml-2 whitespace-nowrap">{documentTypeLabels[record.documentType] ?? record.documentType}</Badge><p className="typo-label mt-0.5 text-slate-800">{record.companyName}</p><p className="typo-body-sm mt-0.5 text-slate-600">{record.delegateName} · {formatDate(record.issueDate)}</p></div><HistoryActions record={record} downloading={downloading} onDetail={onDetail} onView={onView} onDownload={onDownload} /></div><div className="mt-2 flex items-center justify-between gap-3 border-t border-slate-100 pt-2"><span className="typo-meta text-slate-500">{formatDateTime(record.createdAt)}</span><Button className="min-h-10 px-3" variant="outline" onClick={onDownload} disabled={downloading}>{downloading ? <LoaderCircle className="animate-spin" size={16} /> : <Download size={16} />} Descargar</Button></div></article> }
...
function LoadingState() { return <div className="mt-6 space-y-3">{[1, 2, 3].map((item) => <Skeleton key={item} className="h-24 w-full rounded-none" />)}</div> }
function EmptyState({ role, filtered, onClear }: { role: 'ADMIN' | 'DELEGADO'; filtered: boolean; onClear: () => void }) { return <div className="mt-6 border border-dashed border-slate-300 bg-[#fffdf8] p-10 text-center"><FileText className="mx-auto text-slate-400" /><h2 className="mt-4 text-2xl">{filtered ? 'No encontramos documentos con esos filtros.' : 'No hay documentos generados todavía.'}</h2><p className="mt-2 text-sm text-slate-600">{filtered ? 'Probá con otro número, fechas más amplias o limpiá los filtros.' : role === 'DELEGADO' ? 'Los documentos que generes aparecerán acá.' : 'Los documentos generados aparecerán acá.'}</p>{filtered && <Button variant="outline" className="mt-5" onClick={onClear}>Limpiar filtros</Button>}</div> }
function ErrorState({ text, onRetry }: { text: string; onRetry: () => void }) { return <div role="alert" className="mt-6 border border-rose-200 bg-rose-50 p-6"><div className="flex gap-3 text-rose-900"><AlertCircle className="shrink-0" size={20} /><p className="font-semibold">{text}</p></div><Button variant="outline" className="mt-4 border-rose-300 text-rose-900" onClick={onRetry}>Reintentar</Button></div> }
