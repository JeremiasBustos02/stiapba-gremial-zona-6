...
export function HistoryPage({ role, onUseAsBase, openDocumentId, onDocumentOpened }: { role: 'ADMIN' | 'DELEGADO'; onUseAsBase?: (detail: DocumentHistoryDetail) => void; openDocumentId?: string; onDocumentOpened?: () => void }) {
  ...
  const view = async (record: DocumentHistoryRecord) => {
    ...
    try { const result = await regenerateDocument(record.id); const url = URL.createObjectURL(result.blob); if (popup) popup.location.href = url; else window.open(url, '_blank', 'noopener,noreferrer'); window.setTimeout(() => URL.revokeObjectURL(url), 60_000) }
  ...
  return <section className="max-w-[82rem]">
     <header className="border-b border-slate-200 pb-5 sm:flex sm:items-start sm:justify-between sm:gap-8 sm:pb-6">
       ...
       <div className="mt-5 w-full sm:mt-0 sm:w-auto sm:shrink-0"><Button variant="outline" className="min-h-11 w-full sm:w-auto" onClick={() => void exportHistory()} disabled={!historyQuery.data || historyQuery.data.totalElements === 0 || exporting}><FileSpreadsheet size={17} aria-hidden="true" /> {exporting ? <><LoaderCircle className="animate-spin" size={17} aria-hidden="true" /> Exportando...</> : 'Exportar'}</Button><p className="typo-meta mt-1 max-w-56 text-slate-500 sm:text-right">Se exportarán todos los documentos que coinciden con los filtros actuales.</p><p aria-live="polite" className="typo-meta mt-2 tabular-nums text-slate-600 sm:hidden">{historyQuery.data ? `${historyQuery.data.totalElements} ${historyQuery.data.totalElements === 1 ? 'documento' : 'documentos'}` : ''}</p></div>
    </header>
    ...
    {downloadError && <p role="alert" className="mt-4 border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800">{downloadError}</p>}
    {historyQuery.isPending ? <LoadingState /> : historyQuery.isError ? <ErrorState text={errorText} onRetry={() => historyQuery.refetch()} /> : historyQuery.data.content.length === 0 ? <EmptyState role={role} filtered={hasFilters} onClear={clearFilters} /> : <>
       <div className="mt-6 hidden overflow-hidden border border-slate-200 bg-white md:block"><Table><TableHeader><TableRow><TableHead>Número</TableHead><TableHead>Generación</TableHead><TableHead>Tipo</TableHead><TableHead>Empresa</TableHead><TableHead>Delegado</TableHead><TableHead>Fecha permiso</TableHead><TableHead className="text-right">Acciones</TableHead></TableRow></TableHeader><TableBody>{historyQuery.data.content.map((record) => <HistoryRow key={record.id} record={record} downloading={downloadingId === record.id} onDetail={() => setSelected(record)} onDownload={() => download(record)} />)}</TableBody></Table></div>
       <div className="mt-5 divide-y divide-slate-200 border-y border-slate-200 md:hidden">{historyQuery.data.content.map((record) => <HistoryItem key={record.id} record={record} downloading={downloadingId === record.id} onDetail={() => setSelected(record)} onDownload={() => download(record)} />)}</div>
...
function HistoryRow({ record, downloading, onDetail, onDownload }: { record: DocumentHistoryRecord; downloading: boolean; onDetail: () => void; onDownload: () => void }) { return <TableRow><TableCell className="px-3 py-2.5"><button type="button" className="font-bold text-blue-800 underline-offset-4 hover:underline" onClick={onDetail}>{record.publicNumber}</button></TableCell><TableCell className="px-3 py-2.5">{formatDateTime(record.createdAt)}</TableCell><TableCell className="px-3 py-2.5"><Badge className="whitespace-nowrap">{documentTypeLabels[record.documentType] ?? record.documentType}</Badge></TableCell><TableCell className="px-3 py-2.5">{record.companyName}</TableCell><TableCell className="px-3 py-2.5">{record.delegateName}</TableCell><TableCell className="px-3 py-2.5">{formatDate(record.issueDate)}</TableCell><TableCell className="px-3 py-2.5 text-right"><ActionGroup aria-label={`Acciones de ${record.publicNumber}`} className="ml-auto w-auto"><ActionGroupItem className="flex-none"><Button variant="outline" onClick={onDownload} disabled={downloading}>{downloading ? <LoaderCircle className="animate-spin" size={17} /> : <Download size={17} />} Descargar</Button></ActionGroupItem><ActionGroupItem className="flex-none"><Button variant="ghost" aria-label={`Más acciones de ${record.publicNumber}`} className="h-11 w-11 min-w-11 p-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-600" onClick={onDetail}><MoreHorizontal size={19} /></Button></ActionGroupItem><ActionGroupItem className="flex-none"><DisabledEmailAction label="Enviar por mail" /></ActionGroupItem></ActionGroup></TableCell></TableRow> }
function HistoryItem({ record, downloading, onDetail, onDownload }: { record: DocumentHistoryRecord; downloading: boolean; onDetail: () => void; onDownload: () => void }) { return <article className="bg-[#fffdf8] py-3 first:pt-0 last:pb-0"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><button type="button" className="typo-control min-h-11 text-blue-800 underline-offset-4 hover:underline" onClick={onDetail}>{record.publicNumber}</button><Badge className="ml-2 whitespace-nowrap">{documentTypeLabels[record.documentType] ?? record.documentType}</Badge><p className="typo-label mt-0.5 text-slate-800">{record.companyName}</p><p className="typo-body-sm mt-0.5 text-slate-600">{record.delegateName} · {formatDate(record.issueDate)}</p></div><button type="button" aria-label={`Más acciones de ${record.publicNumber}`} className="grid h-11 w-11 min-h-11 min-w-11 shrink-0 place-items-center rounded-md text-slate-500 hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-600" onClick={onDetail}><MoreHorizontal size={19} /></button></div><div className="mt-2 flex items-center justify-between gap-3 border-t border-slate-100 pt-2"><span className="typo-meta text-slate-500">{formatDateTime(record.createdAt)}</span><Button className="min-h-10 px-3" variant="outline" onClick={onDownload} disabled={downloading}>{downloading ? <LoaderCircle className="animate-spin" size={16} /> : <Download size={16} />} Descargar</Button></div></article> }
