...
describe('HistoryPage', () => {
  ...
  it('opens the email dialog and validates the recipient before sending', async () => {
    ...
    await waitFor(() => expect(document.querySelector<HTMLInputElement>('input[type="email"]')).not.toBeNull())
    const dialog = document.querySelector('[role="dialog"]')!
    const sendButton = [...dialog.querySelectorAll('button')].find((button) => button.textContent?.trim() === 'Enviar')
    await act(async () => sendButton!.click())
    await waitFor(() => expect(document.body.textContent).toContain('Ingresá un correo electrónico válido.'))
