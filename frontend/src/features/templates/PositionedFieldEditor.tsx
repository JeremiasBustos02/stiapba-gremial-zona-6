...
export function PositionedFieldEditor({ templateId, variant, onBack }: { templateId: string; variant: TemplateVariant; onBack: () => void }) {
  ...
  const previewSize = previewSizeForWidth(previewWidth, pageSize.width, pageSize.height)
  ...
  useEffect(() => {
    ...
    const updateWidth = () => setPreviewWidth(responsivePreviewWidth(host.clientWidth))
    updateWidth()
  ...
  function upsertField(next: EditableField) {
    setSaveFeedback('')
    setFields((current) => current.some((field) => field.clientId === next.clientId) ? current.map((field) => field.clientId === next.clientId ? next : field) : [...current, next])
  }
  ...
  function previewRect(field: EditableField) {
    return pdfToPreview({ x: field.x ?? 0, y: field.y ?? 0, width: field.width ?? 0, height: field.height ?? 0 }, previewSize.width, previewSize.height, pageSize.width, pageSize.height)
  }
  ...
  function begin(event: React.PointerEvent<HTMLElement>, kind: NonNullable<Interaction>['kind'], rect: PdfRect, fieldId?: string) {
  ...
  function move(event: React.PointerEvent<HTMLDivElement>) {
    ...
    if (interaction.kind === 'create') {
      setDrawingRect({ x: Math.min(start.x, current.x), y: Math.min(start.y, current.y), width: Math.abs(current.x - start.x), height: Math.abs(current.y - start.y) })
      return
    }
    if (!interaction.fieldId) return
    const next = interaction.kind === 'move'
      ? { x: Math.max(0, Math.min(width - rect.width, rect.x + current.x - start.x)), y: Math.max(0, Math.min(height - rect.height, rect.y + current.y - start.y)), width: rect.width, height: rect.height }
      : { x: rect.x, y: rect.y, width: Math.max(minimumSize, Math.min(width - rect.x, rect.width + current.x - start.x)), height: Math.max(minimumSize, Math.min(height - rect.y, rect.height + current.y - start.y)) }
  ...
  function finishInteraction() {
    if (interaction?.kind === 'create' && drawingRect && container.current) {
      if (canCreatePositionedField(drawingRect, minimumSize)) {
        ...
        setFields((current) => [...current, { ...field, ...previewToPdf(drawingRect, previewSize.width, previewSize.height, pageSize.width, pageSize.height) }])
  ...
  function labelFor(id: string) {
    return overlayLabel(definitionsQuery.data?.find((definition) => definition.id === id)?.label)
  }
  ...
  return <main className="min-h-screen overflow-x-hidden bg-slate-100 px-3 py-4 text-slate-900 sm:px-6 sm:py-6">
    <header className="mx-auto mb-5 max-w-[1500px]">
      <button type="button" onClick={onBack} className="min-h-11 text-sm font-semibold text-blue-700">← Volver a la plantilla</button>
      <h1 className="mt-1 text-2xl font-bold">Configurar campos: {variant.nombre}</h1>
    </header>
    {saveFeedback && <p role="status" aria-live="polite" className="mx-auto mb-5 max-w-[1500px] rounded-xl bg-emerald-50 p-4 text-sm font-semibold text-emerald-800">{saveFeedback}</p>}
    {saveError && <p role="alert" className="mx-auto mb-5 max-w-[1500px] rounded-xl bg-rose-50 p-4 text-sm text-rose-800">{saveError}</p>}
    <div className="mx-auto grid max-w-[1500px] grid-cols-1 gap-4 lg:gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
      <section className="min-w-0 rounded-xl bg-slate-300 p-3 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          {hasDetectedFields
            ? <p className="text-sm text-slate-700">Encontramos {discovered.length} campos rellenables en este documento.</p>
            : <p className="max-w-lg text-sm text-slate-700">Este PDF no tiene campos rellenables. Marcá sobre el documento las zonas donde querés que el sistema complete información.</p>}
          {!hasDetectedFields && <button type="button" disabled={drawingMode} onClick={() => { setDrawingMode(true); setSaveFeedback('') }} className="min-h-11 rounded-lg bg-blue-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">+ Agregar campo</button>}
        </div>
        {drawingMode && <p role="status" className="mb-3 rounded-lg bg-blue-50 p-3 text-sm font-semibold text-blue-800">Dibujá un rectángulo sobre el documento</p>}
        {pdfQuery.isPending && <p className="p-12 text-center">Cargando PDF...</p>}
        {pdfQuery.isError && <p role="alert" className="rounded bg-rose-50 p-4 text-rose-800">No pudimos cargar el PDF de la variante.</p>}
        <div ref={previewHost} className="w-full min-w-0 overflow-x-hidden">
          {blobUrl && previewWidth > 0 && <div ref={container} onPointerMove={move} onPointerUp={finishInteraction} onPointerCancel={finishInteraction} style={{ width: previewSize.width }} className="relative mx-auto bg-white shadow-xl">
            <Document file={blobUrl} loading="Cargando PDF..." error="No pudimos mostrar el PDF." onLoadSuccess={({ numPages }) => { setPageCount(numPages); setPageNumber((value) => Math.min(value, numPages)) }}>
              <Page pageNumber={pageNumber} width={previewWidth} renderTextLayer={false} onLoadSuccess={(page) => { const viewport = page.getViewport({ scale: 1 }); setPageSize({ width: viewport.width, height: viewport.height }) }} />
            </Document>
            {hasDetectedFields && discovered.filter((field) => field.pageNumber === pageNumber && field.x !== null && field.y !== null && field.width !== null && field.height !== null).map((field) => {
              const configured = fields.find((value) => value.mode === 'ACROFORM' && value.acroFieldName === field.acroFieldName)
              const rect = pdfToPreview({ x: field.x!, y: field.y!, width: field.width!, height: field.height! }, previewSize.width, previewSize.height, pageSize.width, pageSize.height)
              const active = selectedDocumentField === field.acroFieldName
              return <button key={field.acroFieldName} type="button" onClick={() => setSelectedDocumentField(field.acroFieldName)} style={{ left: rect.x, top: rect.y, width: rect.width, height: rect.height }} className={`absolute z-20 border-2 text-left text-[10px] font-bold ${active ? 'border-blue-800 bg-blue-500/35' : 'border-blue-600 bg-blue-500/15'} text-blue-950`}><span className="absolute -top-5 left-0 rounded bg-blue-800 px-1.5 py-0.5 text-white">{labelFor(configured?.fieldDefinitionId ?? '')}</span></button>
            })}
            {!hasDetectedFields && positioned.map((field) => {
              const rect = previewRect(field)
              return <button key={field.clientId} type="button" onPointerDown={(event) => { setSelectedId(field.clientId); begin(event, 'move', rect, field.clientId) }} style={{ left: rect.x, top: rect.y, width: rect.width, height: rect.height, touchAction: 'none' }} className={`absolute z-20 border-2 text-left text-[10px] font-bold ${selectedId === field.clientId ? 'border-blue-800 bg-blue-500/30' : 'border-blue-600 bg-blue-500/15'} text-blue-950`}><span className="absolute -top-5 left-0 rounded bg-blue-800 px-1.5 py-0.5 text-white">{labelFor(field.fieldDefinitionId)}</span></button>
            })}
            {selected?.mode === 'POSITIONED' && !drawingMode && (() => {
              const rect = previewRect(selected)
              return <button type="button" aria-label="Redimensionar campo" onPointerDown={(event) => begin(event, 'resize', rect, selected.clientId)} style={{ left: rect.x + rect.width - 16, top: rect.y + rect.height - 16, touchAction: 'none' }} className="absolute z-30 grid h-8 w-8 place-items-center"><span className="h-3 w-3 rounded-sm bg-blue-700" /></button>
            })()}
            {drawingRect && <div aria-hidden="true" style={{ left: drawingRect.x, top: drawingRect.y, width: drawingRect.width, height: drawingRect.height }} className="pointer-events-none absolute z-30 border-2 border-dashed border-blue-800 bg-blue-400/25" />}
            {drawingMode && <div aria-label="Área para dibujar un campo" onPointerDown={startDrawing} className="absolute inset-0 z-40 cursor-crosshair touch-none" />}
          </div>}
        </div>
        <div className="mt-4 flex items-center justify-between gap-3">
          <button type="button" disabled={pageNumber <= 1} onClick={() => setPageNumber(pageNumber - 1)} className="min-h-11 rounded border px-3 py-2 text-sm disabled:opacity-50">Anterior</button>
          <span className="text-center text-sm text-slate-600">Página {pageNumber} de {pageCount || '-'}</span>
          <button type="button" disabled={pageNumber >= pageCount} onClick={() => setPageNumber(pageNumber + 1)} className="min-h-11 rounded border px-3 py-2 text-sm disabled:opacity-50">Siguiente</button>
        </div>
      </section>
      <aside className="w-full rounded-xl bg-white p-4 shadow-sm sm:p-5">
        {hasDetectedFields
          ? <AcroformConfiguration names={discovered} fields={fields} definitions={definitionsQuery.data ?? []} selectedName={selectedDocumentField} onSelect={setSelectedDocumentField} onUpsert={upsertField} onRemove={removeField} onCreate={(clientId, acroFieldName) => setNewDataFor({ clientId, acroFieldName })} />
          : <><h2 className="text-lg font-bold">{selected ? 'Campo seleccionado' : 'Campos del documento'}</h2>{selected?.mode === 'POSITIONED' ? <PositionedConfiguration field={selected} definitions={definitionsQuery.data ?? []} onChange={updateField} onCreate={(clientId) => setNewDataFor({ clientId })} onDelete={() => removeField(selected.clientId)} /> : <p className="mt-3 text-sm text-slate-600">Agregá o seleccioná un campo para asignarle el dato que se completará.</p>}</>}
      </aside>
    </div>
    <div className="mx-auto mt-5 grid max-w-[1500px] grid-cols-2 gap-3 sm:flex sm:justify-end">
      <button type="button" onClick={onBack} className="min-h-11 rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold">Cancelar</button>
      <button type="button" disabled={!canSaveFieldConfiguration(dirty, saveMutation.isPending)} onClick={() => saveMutation.mutate()} className="min-h-11 rounded-lg bg-blue-700 px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60">{saveMutation.isPending ? 'Guardando...' : 'Guardar cambios'}</button>
    </div>
