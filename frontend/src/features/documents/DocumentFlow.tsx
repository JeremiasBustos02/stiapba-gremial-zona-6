...
function PageIntro({ title, description, context = 'Nuevo documento', step }: { title: string; description: string; context?: string; step?: string }) {
  ...
  return <header className="max-w-3xl"><div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold uppercase tracking-wide text-blue-700 sm:text-sm sm:normal-case sm:tracking-normal">{context}</p>{step && <><span className="sr-only">{step}</span><span aria-hidden="true" className="hidden rounded-full bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-700 sm:inline">{step}</span></>}</div>{step && <div className="mt-3 sm:hidden"><div aria-hidden="true" className="flex items-center"><i className="h-2.5 w-2.5 rounded-full bg-blue-700" /><span className={`h-0.5 flex-1 ${currentStep >= 2 ? 'bg-blue-700' : 'bg-slate-300'}`} /><i className={`h-2.5 w-2.5 rounded-full ${currentStep >= 2 ? 'bg-blue-700' : 'bg-slate-300'}`} /><span className={`h-0.5 flex-1 ${currentStep >= 3 ? 'bg-blue-700' : 'bg-slate-300'}`} /><i className={`h-2.5 w-2.5 rounded-full ${currentStep >= 3 ? 'bg-blue-700' : 'bg-slate-300'}`} /></div><p className="mt-2 text-xs font-semibold text-slate-600">Paso {currentStep} de 3 · {stepName}</p></div>}<h1 className="mt-1.5 text-2xl font-bold tracking-tight sm:mt-2 sm:text-4xl">{title}</h1><p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-600 sm:mt-3 sm:text-base">{description}</p></header>
}
...
function FormSection({ title, children }: { title: string; children: React.ReactNode }) { return <section className="border-b border-slate-200 py-5 first:pt-0 last:border-b-0"><h2 className="text-xs font-bold uppercase tracking-wide text-slate-600 sm:text-sm">{title}</h2><div className="mt-4">{children}</div></section> }
