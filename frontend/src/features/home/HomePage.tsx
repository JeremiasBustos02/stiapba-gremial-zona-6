...
export function HomePage({ user, onNavigate, onUseAsBase }: {
  ...
  return <section className="max-w-[82rem]">
    ...
    {dashboardQuery.isPending ? <DashboardLoading /> : dashboardQuery.isError ? <DashboardError text={errorText} onRetry={() => dashboardQuery.refetch()} /> : <>
      {dashboardQuery.data.latestDocument ? <LatestDocument document={dashboardQuery.data.latestDocument} onOpen={openDetail} onNavigate={onNavigate} /> : <EmptyDashboard onCreate={() => onNavigate('new-document')} />}
      {dashboardQuery.data.totalDocuments > 0 && <div className="mt-10 grid gap-10 border-t border-slate-200 pt-8 lg:mt-12 lg:grid-cols-[minmax(0,1fr)_18rem] lg:gap-10">
        <RecentActivity documents={dashboardQuery.data.recentActivity} onOpen={openDetail} onHistory={() => onNavigate('history')} />
        <Metrics documentsThisMonth={dashboardQuery.data.documentsThisMonth} totalDocuments={dashboardQuery.data.totalDocuments} role={user.role} />
      </div>}
    </>}
...
function RecentActivity({ documents, onOpen, onHistory }: { documents: DashboardDocument[]; onOpen: (document: DashboardDocument) => void; onHistory: () => void }) {
  return <section className="motion-reveal [animation-delay:160ms]" aria-labelledby="recent-activity-title"><div className="flex items-end justify-between gap-4"><div><p className="typo-eyebrow text-slate-500">Registro de trabajo</p><h2 id="recent-activity-title" className="typo-heading-2 mt-2">Actividad reciente</h2></div><button type="button" onClick={onHistory} className="hidden min-h-11 items-center gap-1 text-sm font-semibold text-blue-800 hover:text-blue-950 sm:flex">Ver todo <ChevronRight size={17} aria-hidden="true" /></button></div>{documents.length ? <ol className="mt-5 border-l border-slate-300">{documents.map((document) => <li key={document.id} className="relative pl-6 pb-6 last:pb-0"><span aria-hidden="true" className="absolute -left-[0.3rem] top-1 h-2.5 w-2.5 rounded-full border-2 border-slate-50 bg-blue-700" /><button type="button" onClick={() => onOpen(document)} className="typo-control text-left text-blue-900 underline-offset-4 hover:underline">{document.publicNumber}</button><p className="typo-body-sm mt-1 text-slate-700">{document.companyName}</p><p className="typo-meta mt-1 text-slate-500"><time dateTime={document.createdAt} title={exactDate(document.createdAt)}>{relativeTime(document.createdAt)}</time></p></li>)}</ol> : <p className="mt-5 text-sm text-slate-600">Cuando generes documentos, aparecerán acá.</p>}<button type="button" onClick={onHistory} className="mt-6 flex min-h-11 items-center gap-1 text-sm font-semibold text-blue-800 sm:hidden">Ver todo el historial <ChevronRight size={17} aria-hidden="true" /></button></section>
}
...
function Metrics({ documentsThisMonth, totalDocuments, role }: { documentsThisMonth: number; totalDocuments: number; role: AuthUser['role'] }) {
  return <aside className="motion-reveal border-t border-slate-200 pt-6 [animation-delay:220ms] lg:border-l lg:border-t-0 lg:pl-7 lg:pt-0" aria-labelledby="metrics-title"><p id="metrics-title" className="typo-eyebrow text-slate-500">{role === 'ADMIN' ? 'Actividad del sistema' : 'Tu actividad'}</p><dl className="mt-5 grid grid-cols-2 gap-x-5 gap-y-6 lg:block lg:space-y-7"><Metric label="Este mes" value={documentsThisMonth} description="documentos generados" /><Metric label="Total útil" value={totalDocuments} description="documentos en historial" /></dl></aside>
}
