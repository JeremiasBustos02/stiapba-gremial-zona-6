...
export function GlobalSearch({ role, onNavigate, onOpenDocument, onPrefill }: Props) {
  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={(next) => next ? setOpen(true) : close()}>
      <DialogContent className="!top-[max(1rem,env(safe-area-inset-top))] !max-h-[calc(100dvh-2rem)] !-translate-y-0 flex flex-col overflow-hidden p-0 sm:!top-[max(2rem,env(safe-area-inset-top))] sm:!max-h-[calc(100dvh-4rem)] sm:max-w-2xl">
