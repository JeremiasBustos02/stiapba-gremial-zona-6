...
type Screen = 'login' | 'first-login' | 'home' | 'new-document' | 'variants' | 'form' | 'preview' | 'profile' | 'admin' | 'users' | 'companies' | 'agreements' | 'templates' | 'positioned-editor'
...
function App() {
  ...
  if (currentUser.role !== 'ADMIN' && ['admin', 'users', 'companies', 'agreements', 'templates', 'positioned-editor'].includes(screen)) {
...
const screenParents: Partial<Record<Screen, Screen>> = {
  profile: 'home', admin: 'home', users: 'admin', companies: 'admin', agreements: 'admin', templates: 'admin', 'new-document': 'home', variants: 'new-document', form: 'variants', preview: 'form',
}

const screenLabels: Partial<Record<Screen, string>> = {
  home: 'Inicio', profile: 'Mi perfil', admin: 'Administración', users: 'Usuarios', companies: 'Empresas', agreements: 'Convenios', templates: 'Plantillas', 'new-document': 'Nuevo documento', variants: 'Versión del documento', form: 'Permiso Gremial', preview: 'Vista previa',
}
...
function TemplateSelection({ onBack, onSelect }: { onBack: () => void; onSelect: () => void }) { const [status, setStatus] = useState<'success' | 'empty' | 'error'>('success'); return <><PageIntro title="Seleccionar documento" description="Elegí el tipo de documento que necesitás generar." /><div className="mt-6 flex gap-2 text-xs"><button onClick={() => setStatus('success')} className="underline">Datos</button><button onClick={() => setStatus('empty')} className="underline">Vacío</button><button onClick={() => setStatus('error')} className="underline">Error</button></div><section className="mt-4 max-w-3xl">{status === 'success' && <button onClick={onSelect} className="flex w-full items-center gap-4 rounded-2xl border border-blue-200 bg-white p-5 text-left shadow-sm hover:border-blue-500"><span className="rounded-xl bg-blue-700 p-3 text-white"><FileText /></span><span className="flex-1"><strong className="text-lg">Permiso Gremial</strong><span className="mt-1 block text-sm text-slate-600">Autorización para actividades sindicales fuera del trabajo.</span></span><ChevronRight className="text-blue-700" /></button>}{status === 'empty' && <EmptyState title="No hay documentos disponibles" text="Todavía no hay tipos de documento activos para utilizar." />}{status === 'error' && <ErrorState text="No pudimos cargar los documentos. Intentá nuevamente." onRetry={() => setStatus('success')} />}</section><button onClick={onBack} className="mt-8 text-sm font-semibold text-slate-600 hover:text-slate-950">Cancelar</button></> }
function VariantSelection({ onBack, onSelect }: { onBack: () => void; onSelect: (value: string) => void }) { return <><PageIntro title="Versión del documento" description="Elegí la firma que se utilizará en el Permiso Gremial." /><section className="mt-8 max-w-3xl space-y-3"><button onClick={() => onSelect('bruna')} className="flex w-full items-center gap-4 rounded-2xl border border-blue-200 bg-white p-5 text-left shadow-sm hover:border-blue-500"><span className="rounded-xl bg-blue-50 p-3 text-blue-700"><Pencil /></span><span className="flex-1"><strong className="text-lg">Bruna</strong><span className="mt-1 block text-sm text-slate-600">Versión activa del documento.</span></span><ChevronRight className="text-blue-700" /></button></section><button onClick={onBack} className="mt-8 text-sm font-semibold text-slate-600">Volver</button></> }

function PermitForm({ form, onChange, onBack, onPreview }: { form: FormData; onChange: (form: FormData) => void; onBack: () => void; onPreview: () => void }) { const [errors, setErrors] = useState<string[]>([]); const delegate = delegates.find((item) => item.value === form.delegate)!; const set = (key: keyof FormData, value: string) => onChange({ ...form, [key]: value }); const submit = (event: React.FormEvent) => { event.preventDefault(); const missing = Object.entries(form).filter(([key, value]) => key !== 'variant' && !value).map(([key]) => key); setErrors(missing); if (!missing.length) onPreview() }; return <><PageIntro title="Permiso Gremial" description="Completá los datos necesarios para generar la vista previa." /><form onSubmit={submit} className="mt-8 max-w-3xl space-y-5"><FormSection title="Contexto institucional"><SelectField label="Provincia" value={form.province} onChange={(value) => set('province', value)} options={[['', 'Seleccioná una provincia'], ['buenos-aires', 'Buenos Aires'], ['cordoba', 'Córdoba']]} error={errors.includes('province') ? 'Seleccioná una provincia.' : undefined} /><SelectField label="Empresa" value={form.company} onChange={(value) => set('company', value)} options={[['', 'Buscar o seleccionar empresa'], ['alimentos-sur', 'Alimentos del Sur S.A.'], ['frigorifico', 'Frigorífico Regional S.A.']]} error={errors.includes('company') ? 'Seleccioná una empresa.' : undefined} /></FormSection><FormSection title="Información del delegado"><SelectField label="Delegado" value={form.delegate} onChange={(value) => set('delegate', value)} options={delegates.map((item) => [item.value, item.name])} /><div className="mt-4 rounded-xl bg-slate-100 px-4 py-3"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">DNI del delegado</p><p className="mt-1 font-medium text-slate-800">{delegate.dni}</p></div></FormSection><FormSection title="Datos del permiso"><label className="block text-sm font-semibold">Fecha de emisión<input required type="date" value={form.issueDate} onChange={(event) => set('issueDate', event.target.value)} className="mt-2 h-12 w-full rounded-xl border border-slate-300 bg-white px-3 outline-none focus:border-blue-700 focus:ring-2 focus:ring-blue-100" /></label><label className="mt-4 block text-sm font-semibold">Día del permiso<input required min="1" max="31" inputMode="numeric" type="number" value={form.permitDay} onChange={(event) => set('permitDay', event.target.value)} placeholder="Ej. 18" className="mt-2 h-12 w-full rounded-xl border border-slate-300 bg-white px-3 outline-none focus:border-blue-700 focus:ring-2 focus:ring-blue-100" />{errors.includes('permitDay') && <span className="mt-1 block text-sm text-rose-700">Indicá el día del permiso.</span>}</label><SelectField label="Convenio" value={form.agreement} onChange={(value) => set('agreement', value)} options={[['', 'Buscar o seleccionar convenio'], ['cct-244', 'CCT-244 - Industria de la alimentación'], ['cct-329', 'CCT-329 - Actividad frigorífica']]} error={errors.includes('agreement') ? 'Seleccioná un convenio.' : undefined} /><div className="mt-4 rounded-xl border border-blue-100 bg-blue-50 px-4 py-3"><p className="text-xs font-semibold uppercase tracking-wide text-blue-700">Versión seleccionada</p><p className="mt-1 font-medium text-slate-900">Bruna</p></div></FormSection><div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" onClick={onBack} className="rounded-xl px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-200">Volver</button><button className="flex items-center justify-center gap-2 rounded-xl bg-blue-700 px-6 py-3 font-semibold text-white hover:bg-blue-800"><Eye size={19} /> Generar vista previa</button></div></form></> }
function FormSection({ title, children }: { title: string; children: React.ReactNode }) { return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><h2 className="text-sm font-bold uppercase tracking-wide text-slate-600">{title}</h2><div className="mt-5">{children}</div></section> }
function SelectField({ label, value, onChange, options, error }: { label: string; value: string; onChange: (value: string) => void; options: string[][]; error?: string }) { return <label className="mt-4 block text-sm font-semibold first:mt-0">{label}<select required value={value} onChange={(event) => onChange(event.target.value)} className="mt-2 h-12 w-full rounded-xl border border-slate-300 bg-white px-3 outline-none focus:border-blue-700 focus:ring-2 focus:ring-blue-100">{options.map(([id, text]) => <option key={id} value={id}>{text}</option>)}</select>{error && <span className="mt-1 block text-sm text-rose-700">{error}</span>}</label> }
...
function AdminPage({ onNavigate }: { onNavigate: (screen: Screen) => void }) { const cards: { title: string; description: string; icon: typeof Users; screen: Screen }[] = [{ title: 'Usuarios', description: 'Personas, roles y accesos.', icon: Users, screen: 'users' }, { title: 'Empresas', description: 'Empresas disponibles.', icon: Landmark, screen: 'companies' }, { title: 'Convenios', description: 'Códigos y descripciones.', icon: FileText, screen: 'agreements' }, { title: 'Plantillas', description: 'Tipos de documento y sus variantes PDF.', icon: FileText, screen: 'templates' }]; return <><PageIntro title="Administración" description="Gestioná los datos necesarios para generar documentos." /><section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{cards.map(({ title, description, icon: Icon, screen }) => <button key={title} onClick={() => onNavigate(screen)} className="rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm hover:border-blue-300 hover:shadow-md"><Icon className="text-blue-700" /><h2 className="mt-6 text-lg font-bold">{title}</h2><p className="mt-2 text-sm text-slate-600">{description}</p><span className="mt-5 flex items-center gap-1 text-sm font-semibold text-blue-700">Abrir <ChevronRight size={16} /></span></button>)}</section></> }
...
function BrandMark({ small = false }: { small?: boolean }) { return <span className={`inline-flex items-center justify-center rounded-xl bg-blue-700 text-white ${small ? 'h-8 w-8 rounded-lg' : 'h-16 w-16'}`}><FileText size={small ? 18 : 32} /></span> }
