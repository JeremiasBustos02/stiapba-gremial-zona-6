...
function App() {
  const [screen, setScreen] = useState<'users' | 'companies' | 'agreements' | 'templates'>('users')
  return <QueryClientProvider client={queryClient}><div className="border-b border-slate-200 bg-white px-4 py-3"><nav className="mx-auto flex max-w-6xl gap-2 overflow-x-auto" aria-label="Administración"><button onClick={() => setScreen('users')} className="rounded-lg px-3 py-2 text-sm font-semibold hover:bg-slate-100">Usuarios</button><button onClick={() => setScreen('companies')} className="rounded-lg px-3 py-2 text-sm font-semibold hover:bg-slate-100">Empresas</button><button onClick={() => setScreen('agreements')} className="rounded-lg px-3 py-2 text-sm font-semibold hover:bg-slate-100">Convenios</button><button onClick={() => setScreen('templates')} className="rounded-lg px-3 py-2 text-sm font-semibold hover:bg-slate-100">Plantillas</button></nav></div>{screen === 'users' && <UserManagementPage />}{screen === 'companies' && <CatalogManagementPage<Company, CompanyForm> kind="companies" title="Empresas" description="Administrá las empresas disponibles para completar documentos." emptyForm={{ nombre: '' }} getItems={getCompanies} createItem={createCompany} updateItem={updateCompany} setActive={setCompanyActive} />}{screen === 'agreements' && <CatalogManagementPage<Agreement, AgreementForm> kind="agreements" title="Convenios" description="Administrá los convenios disponibles para completar documentos." emptyForm={{ codigo: '', descripcion: '' }} getItems={getAgreements} createItem={createAgreement} updateItem={updateAgreement} setActive={setAgreementActive} />}{screen === 'templates' && <TemplateManagementPage />}</QueryClientProvider>
}
